"""저장소를 **파일시스템과 같은 모양으로** 얹는다.

## 왜 공통 부모를 얹는가

페이지가 이 저장소의 `dist` 와 레지스트리의 매니페스트를 함께 싣는다. 각각을 다른
접두사로 얹으면 컴파일러가 뱉은 상대 경로가 어긋난다 — URL 모양을 파일 모양과 같게
두면 그 전제가 그대로 맞는다.

## 코어는 더 이상 옆에서 오지 않는다

전에는 `../borch` 도 얹었다. 하네스가 코어의 벤치 모듈을 쓰는데 그것이 npm 배포에
없어서, **옆에 받아둔 코어를 싣는 수밖에 없었다.** 코어가 `dist/test/bench` 와
`accuracy` 를 싣기 시작하면서(0.2.2) 그 이유가 사라졌다 — 이제 하네스도 `node_modules`
에서 온다. 코어가 두 벌이 될 걱정도 같이 사라졌다.

## 그렇다고 부모 전체를 열지는 않는다

공통 부모에는 다른 저장소들도 있다. 127.0.0.1 에만 묶이는 임시 서버라도 필요 없는
것을 여는 이유는 없으므로, 맨 앞 칸이 허용한 것이 아니면 거절한다.
"""
