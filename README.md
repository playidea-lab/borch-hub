...
**그리고 실은 뒤에 샘플로 대 본다.** 해시는 *받은 바이트가 맞다*는 말이지 *이
브라우저에서 그 모델이 나온다*는 말이 아니다. 둘이 벌어지는 것을 실측했다 —
EfficientNet-B4 는 바이트도 층도 맞는데 이 장치에서 로짓이 전부 0 이었다
([borch#122](https://github.com/playidea-lab/borch/issues/122)). 그때 `load` 는 아무
말 없이 그 모델을 건넸다.
